...
public class MemberController {
	...
	public String login(MemberVO bag, HttpSession session) {
		...
		if (result == 1) {
			...
			// 기존에 로그인을 시도했던 페이지, member.jsp로 가고싶은 경우 redirect 를 사용합니다.
	...
	public void insert(MemberVO bag) {
		/*
		 * 메서드의 입력변수로 변수를 선언해두면 컨트롤러내의 메서드 내에서는
		 * 
		 * 1) Bag을 만들어서 2) 클라이언트로부터 전달된 데이터도 받아줌 3) bag에 데이터 다 넣어줌
		 */
	...
	@RequestMapping("one5")
	public void oneMember(String id, Model model) {
		System.out.println("Search a User Info");
